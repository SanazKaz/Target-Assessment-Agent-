...
class PubMedProcessor_sync:
    def __init__(self, email, timeout=300):
        load_dotenv()
        self.email = email
        Entrez.email = email
    ...
    def full_process(self, pubmed_query, doc_query, max_results=10):
        start_time = time.time()
        
        def check_timeout():
            if time.time() - start_time > self.timeout:
                raise TimeoutError("Process exceeded the time limit")
...
if __name__ == "__main__":
    async def main():
        processor = PubMedProcessor_sync(email="your_email@example.com")
